-- Migration: สร้างระบบขาย-คลัง-จัดส่งแบบครบวงจร
-- Created: 2025-09-29
...
CREATE TYPE public.order_status AS ENUM (
    ...
    'confirmed',       -- ยืนยันแล้ว (รอคลังจัดเก็บ)
    'picking',         -- กำลังจัดเก็บ
    'packed',          -- แพ็คเสร็จ (พร้อมจัดส่ง)
...
CREATE TABLE IF NOT EXISTS public.sales_orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    order_number VARCHAR(50) UNIQUE NOT NULL, -- เลขที่ใบสั่งซื้อ (auto generate)
    ...
    required_date DATE, -- วันที่ต้องการสินค้า
    shipped_date DATE,  -- วันที่จัดส่ง
    delivered_date DATE, -- วันที่ส่งถึง
...
CREATE TABLE IF NOT EXISTS public.sales_order_items (
    ...
    -- ข้อมูลสินค้า (เก็บไว้ในกรณี product ถูกลบ)
    product_name VARCHAR(255) NOT NULL,
    product_sku VARCHAR(100),

    -- จำนวนและราคา
    quantity_cartons INTEGER DEFAULT 0,  -- จำนวนลัง
    quantity_boxes INTEGER DEFAULT 0,    -- จำนวนกล่อง
    quantity_pieces INTEGER DEFAULT 0,   -- จำนวนชิ้น
    total_pieces INTEGER NOT NULL,       -- รวมทั้งหมดเป็นชิ้น

    unit_price DECIMAL(15,2) NOT NULL,   -- ราคาต่อชิ้น
    line_total DECIMAL(15,2) NOT NULL,   -- รวมเงินรายการ
...
-- Index
CREATE INDEX IF NOT EXISTS idx_order_status_history_order ON public.sales_order_status_history(order_id);
CREATE INDEX IF NOT EXISTS idx_order_status_history_date ON public.sales_order_status_history(changed_at);
...
CREATE TABLE IF NOT EXISTS public.warehouse_tasks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    order_id UUID NOT NULL REFERENCES public.sales_orders(id) ON DELETE CASCADE,
    task_type VARCHAR(50) NOT NULL DEFAULT 'picking', -- picking, packing, shipping
    status public.warehouse_task_status NOT NULL DEFAULT 'pending',
    priority INTEGER DEFAULT 3, -- 1=สูง, 3=ปกติ, 5=ต่ำ
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_warehouse_tasks_order ON public.warehouse_tasks(order_id);
...
BEGIN
    -- Format: SO2025090001, SO2025090002, etc.
...
-- View สำหรับใบสั่งซื้อพร้อมข้อมูลลูกค้า (Basic version without auth.users dependencies)
...
-- View สำหรับรายการสินค้าในใบสั่งซื้อ (Basic version without products table dependency)
CREATE OR REPLACE VIEW public.sales_order_items_detail AS
SELECT
    soi.*,
    so.order_number,
...
-- สำเร็จ!
SELECT 'Sales System Database Schema created successfully!' as status;