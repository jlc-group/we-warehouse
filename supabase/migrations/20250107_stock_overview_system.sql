-- Stock Overview System
-- Provides high-performance cached view of warehouse inventory
-- with summary statistics and detailed SKU breakdown

-- Main function: Get stock overview with summary stats and item details
...
CREATE INDEX IF NOT EXISTS idx_inventory_items_sku_location
ON inventory_items(sku, location);

CREATE INDEX IF NOT EXISTS idx_products_sku_code
ON products(sku_code) WHERE is_active = true;

-- Grant execute permission
GRANT EXECUTE ON FUNCTION get_stock_overview(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_stock_overview(UUID) TO anon;

-- Comment
COMMENT ON FUNCTION get_stock_overview(UUID) IS
'Returns comprehensive stock overview with summary statistics and detailed SKU breakdown.
Optimized for caching with 3-hour refresh interval.';
