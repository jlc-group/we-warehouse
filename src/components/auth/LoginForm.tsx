...
const loginSchema = z.object({
  emailOrUsername: z.string().min(1, 'กรุณากรอกชื่อผู้ใช้หรืออีเมล'),
...
export function LoginForm({ onSwitchToRegister }: LoginFormProps) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      ...
      if (data.remember) {
        localStorage.setItem('remember_user', data.emailOrUsername);
      } else {
  ...
  useEffect(() => {
    ...
    if (rememberedUser) {
      setValue('emailOrUsername', rememberedUser);
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="emailOrUsername">ชื่อผู้ใช้ หรือ อีเมล</Label>
              <Input
                id="emailOrUsername"
                type="text"
                placeholder="admin หรือ admin@warehouse.local"
            ...
            <div className="text-center">
              ...
              {showDemoInfo && (
                <div className="bg-blue-50 p-3 rounded-lg text-left text-xs space-y-2 mb-2">
                  <p className="font-semibold text-blue-800">ผู้ใช้ทดสอบ (รหัสผ่าน: อะไรก็ได้)</p>
                  <div className="space-y-1">
                    <p><strong>admin</strong> หรือ <strong>admin@warehouse.local</strong> - ผู้ดูแลระบบ (ระดับ 5)</p>
                  </div>
                  <div className="mt-2 space-x-1">
                    {[
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setValue('emailOrUsername', username);
